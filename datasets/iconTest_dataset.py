...
class IconTestDataset(H5PYDataset):
...
#            path = find_in_data_path(self._filename)
            path = "/root/HDML/datasets/data/cars196/iconTest.hdf5"
        except IOError as e:
            msg = str(e) + (""".
...
def load_as_ndarray(which_sets=['train', 'test']):
    ...
    for split in which_sets:
        data = IconTestDataset([split], load_in_memory=True).data_sources
        datasets.append(data)
    return datasets


if __name__ == '__main__':
    dataset = IconTestDataset(['train'])
