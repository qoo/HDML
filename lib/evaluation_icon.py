...
def Evaluation_icon(stream_test, image_mean, sess, x_raw, label_raw, is_Training, embedding, num_class, neighb):
    y_batches = []
    c_batches = []
    for batch in tqdm(copy.copy(stream_test.get_epoch_iterator())):
        x_batch_data, c_batch_data = batch
...
def Evaluation_icon2(Evaluation_icon2, image_mean, sess, x_raw, label_raw, is_Training, embedding, num_class, neighb):
    ...
    images = images_[0]
    labels = labels_[0]
    for idx in range(0, len(labels),128):
        image = images[idx:idx+128]
        x_batch_data, c_batch_data = image, labels[idx:idx+128]
